...
- **Status:** ✅ FIXED (guard added to confirmed crash site + PDF-bytes validation for the single-tap path; same audit found and fixed 5 more instances of the identical gap) - resubmitting to App Review as v2.0.2+21 (v2.0.1+20 never uploaded; superseded by a deployment-target fix Transporter flagged) without hardware reproduction on the exact crash-report device (iPad Air 11" M3, unavailable) or any local repro; see "Decision: Resubmitting Without Hardware Reproduction" in CRASH-001-stamp-print-race-condition.md
- **Priority:** CRITICAL
