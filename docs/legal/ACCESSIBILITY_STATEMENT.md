...
- ✅ Customer app wallet home screen's new sort control (DECISION-024, built 2026-09-08 on `feature/customer-wallet-card-sorting` - not yet merged) was built accessible from the start rather than needing a retrofit: the trigger icon's accessible label states the current sort order (e.g. "Sort (currently Newest First)"), not just a static "Sort" - a sighted user can at least remember or glance at the list, but a VoiceOver/TalkBack user has no equivalent fallback, so the label says outright what would otherwise require opening the menu to check. Each option in the menu itself uses Flutter's `CheckedPopupMenuItem`, which exposes proper checked/unchecked state to assistive technology automatically (`SemanticsRole.menuItemCheckbox`) - no custom semantics work needed there. Covered by `test/screens/customer_home_sort_test.dart`, including a dedicated test asserting on the accessible label text itself, not just visible behavior.
