...
**Status: SHIPPED TO TESTFLIGHT (TEST-016 through TEST-020), superseded by `APP_STORE_METADATA_PACKET_v2_1_0_27.md` for App Store submission.** Build 26 was built and uploaded to TestFlight before TEST-021 was found; since Apple doesn't allow re-uploading a build number with different content, the actual submission candidate is v2.1.0+27 - see that packet.

**Supersedes:** `APP_STORE_METADATA_PACKET_v2_0_4_24.md` (never built, folded into this release along with the interim test-only build v2.0.4+25) and, more importantly, `APP_STORE_METADATA_PACKET_v2_0_3_23.md` - that build is **live on the App Store** and contains TEST-016 (businesses with 3 or 4 required stamps can never issue a valid card). This build fixes that, plus a redemption QR-capacity failure and its issue-card counterpart (TEST-017 through TEST-021) found while testing the TEST-016 fix - see `docs/project-management/DEFECT_TRACKER.md` for full detail on all five. Everything else (Category, Subtitle, Description, Keywords, Promotional Text, App Review Notes) is **unchanged from the v2_0_3_23 packet** - see that file for the full text of those fields. Only What's New changes below.
