...
**v2.2.1+36 (built, uploaded to TestFlight, TestFlight-tested — not yet submitted for App Store review):**
...
- [ ] **All code merged to `develop`** — not yet; currently on `fix/ios-print-sharepdf-hang` (branched from `feature/android-port`, not merged anywhere)
- [ ] **`develop` merged to `main` branch** — not yet, blocked on the above
- [ ] **Release branch created** — not yet
...
- [x] **`flutter analyze` clean** of errors on all three packages (pre-existing lint-level info/warnings remain, unchanged by this work)
- [ ] **Metadata entered into App Store Connect** — drafted in `APP_STORE_METADATA_PACKET_v2_2_1_36.md` 2026-08-27, not yet entered into ASC
- [ ] **Submitted for App Store review** — not yet; pending merge to `develop`
- [ ] **Approved and released** — pending submission
- [x] **v2.2.0+32 status** — already approved and released 2026-08-24; v2.2.1+36 will supersede it once submitted and approved, as a reliability/bug-fix release (no new App-Store-facing capability)
...
- [ ] **v2.2.1+36 What's New, App Review Notes** — drafted 2026-08-27 in [`APP_STORE_METADATA_PACKET_v2_2_1_36.md`](APP_STORE_METADATA_PACKET_v2_2_1_36.md), not yet entered into ASC. Subtitle/Keywords/Description unchanged from v2.2.0+32. **Promotional Text: re-verify directly in ASC before submitting even though it's "unchanged"** - it's been found blank on two prior releases (v2.0.3+23, v2.1.1+29) despite being documented as already-live.
...
2. [ ] **Select build** for Customer app submission — pending submission
3. [ ] **Select build** for Supplier app submission — pending submission
4. [ ] **Complete all required fields** in App Store Connect — metadata drafted in `APP_STORE_METADATA_PACKET_v2_2_1_36.md` 2026-08-27, not yet entered into ASC
5. [ ] **Screenshots** — expected unchanged (the print flow's UI change - share sheet instead of a direct print-preview panel - isn't part of the staged screenshot set, which doesn't show the print dialog itself), worth a quick visual sanity check before submitting anyway
6. [ ] **Submit for review** — pending: merge `fix/ios-print-sharepdf-hang` to `develop`, then `develop` to `main`, cut a release branch, then submit
7. [ ] **Monitor review status**
8. [ ] **Respond to App Review** if questions arise
9. [ ] **Release approved apps**
