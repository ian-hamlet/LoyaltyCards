...
class _CustomerCardDetailState extends State<CustomerCardDetail> {
        ...
        child: Column(
          children: [
                            ...
                            child: Center(
                              child: Padding(
                                padding: const EdgeInsets.symmetric(
                                    vertical:
                                        12), // Horizontal space around rotated text
                                child: RotatedBox(
                                  quarterTurns: 3,
                                  child: Row(
            ...
            if (_card!.mode == OperationMode.simple) ...[
                        ...
                        child: Column(
                          ...
                          children: [
                            const Icon(Icons.check_circle,
                                size: 64, color: BrandColors.success),
                            const SizedBox(height: 12),
                            const Text(
                              'Card Redeemed!',
                              style: TextStyle(
              ...
              if (_card!.isRedeemed)
                Container(
                  ...
                  child: Column(
                    children: [
                      const Icon(Icons.check_circle,
                          size: 80, color: BrandColors.success),
                      const SizedBox(height: 16),
                      const Text(
                        'Card Redeemed!',
                        style: TextStyle(
                ...
                Container(
                  padding: const EdgeInsets.all(
                      8), // Reduced from 16 (TEST-010 - Compact QR Layout)
                  decoration: BoxDecoration(
                    color: Colors.white,
                    borderRadius: BorderRadius.circular(20),
                  ...
                  child: QrImageView.withQr(
                    ...
                    size: QRCodeSize.calculate(context) *
                        0.95, // 95% size (TEST-010 - Compact QR Layout)
              ...
              const SizedBox(
                  height: 6), // Reduced from 12 (TEST-010 - Compact QR Layout)
  ...
  Widget _buildStampGrid(Color brandColor) {
    // ExcludeSemantics: same reasoning as the missing per-stamp numbers
    // above - "N of M stamps" in _buildStampBadge already says this
    // accessibly. Without this, VoiceOver/TalkBack would stop on
    // stampsRequired individual unlabeled circles for no extra information.
          ...
          return Wrap(
            ...
            children: List.generate(_card!.stampsRequired, (index) {
              ...
              return Container(
                ...
                child: isCollected
                    ? Icon(Icons.check,
                        color: brandColor, size: stampSize * 0.56)
                    : null,
              );
            }),
          );
  ...
  Widget _buildCardCreationItem() {
    ...
    final initialStampCount = _stamps.where((stamp) {
      final timeDiff =
          stamp.timestamp.difference(_card!.createdAt).inSeconds.abs();
