...
- **A `stampsRequired` increase that completed a card via an ordinary stamp scan never took effect on the next card.** Reported by the user: raise the requirement mid-collection, and the very next scan completing that card (correctly, at the original, lower requirement) auto-created a fresh next card that still carried the old requirement too - the increase was silently lost, not just delayed. Two of the three places a new card gets created after a completion had been fixed to pick up a pending increase; the third - completion reached via an ordinary stamp scan, not a separate redemption - had been missed. Full detail: `docs/project-management/DEFECT_TRACKER.md` DECISION-021.
