...
- **Target Build:** v2.2.0+31 (build-only bump, same not-yet-shipped 2.2.0 line as DECISION-020 above)
- **Notes:** Branch `feature/businessedit`, not yet merged to `develop`/`main` - explicitly held pending further instruction. Full design discussion, including the rejected alternatives for the `stampsRequired` propagation question, lives in `docs/project-management/Requirements/DISCUSSION_Business_Field_Editing.md`.
