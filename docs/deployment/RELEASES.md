...
### v2.2.4+40 - Build 40 (🟡 Release branch cut, not yet submitted to either store)
- **Date:** September 4, 2026 (Android merged to `develop` and real-device tested; release branch cut, iOS IPAs and macOS app built and verified, same day)
- **Platform:** First release branch to cover iOS and Android together.
  - **Android:** both apps already live on Google Play **Internal testing** (not TestFlight - Play's equivalent, no review gate) - installed and functionally tested (Express + Secure Mode issue/stamp/redeem, biometric-gated Recovery Backup/Clone to Another Device) on two real Samsung Galaxy devices (A14/A12), the project's first real Android hardware. This is `feature/android-port`'s first release since merging to `develop`.
  - **iOS:** both apps built via `build_both_apps_ios.sh` and verified (valid IPAs, correct 2.2.4/40 in `Info.plist`, correct bundle IDs) - not yet uploaded to TestFlight.
  - **macOS:** Supplier app release build verified (`flutter build macos --release` - universal x86_64+arm64 binary, correctly signed, correct version embedded).
- **Branch:** merged `feature/android-port` → `develop` (`767f641`, explicit merge commit - `develop` had not diverged at all, zero conflicts), release branch `releases/v2.2.4-build40` cut from `develop`. Not yet merged to `main`.
- **Version:** 2.2.4+40
- **Status:** 🟡 In progress - Android already real-device tested via Play Console Internal testing; iOS/macOS builds verified locally but not yet uploaded to TestFlight. Not yet submitted to either store for review.
- **Focus:** The full Android port (native build/signing/release pipeline, real app icons/display names replacing Flutter's placeholders, Play Store metadata/graphics/submission checklist drafted) plus three real bugs found and fixed along the way: two Android-only (a biometric-auth crash needing `FlutterFragmentActivity`, an anti-fraud device-signal bug that hashed a non-per-device OS build tag instead of a real per-install identifier) and one affecting both platforms equally, just never visible on iOS screens before (a Secure Mode redemption screen showing two buttons that did the same thing). Also fixed, this session: a leftover Intel Homebrew `openssl` shadowing the native arm64 one, which was blocking every iOS/macOS build on this machine (`openssl x509 -subject` failing without Rosetta) - disabled the stale symlink rather than installing Rosetta as a workaround, and downloaded Xcode's missing iOS 26.5 device platform component. Full detail: `source/shared/lib/version.dart` Builds 37-40, `docs/project-management/ANDROID_PORT_PLAN.md`.
- **Testing:** shared 216/216, customer_app 184/184 (+8 skipped), supplier_app 151/151 (+4 skipped). `flutter analyze` clean on both apps. Android: full Express/Secure Mode real-device pass on two Samsung Galaxy devices, 2026-09-04. iOS/macOS: build verification only so far (valid IPAs/app, correct version embedded) - TestFlight testing still pending.
- **Next Steps:** Upload both iOS IPAs to TestFlight via Transporter and TestFlight-test on a real iPhone/iPad; complete the Play Store listing in Play Console (content rating questionnaire, Data Safety form - both already decided, see `PLAY_STORE_METADATA_PACKET_v2_2_2_37.md` and `ANDROID_APP_SUBMISSION_CHECKLIST.md`); then submit both platforms for store review.
