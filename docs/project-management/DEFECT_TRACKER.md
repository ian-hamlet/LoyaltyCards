...
- **Testing Verified:** New `qr_tokens_test.dart` additions (5 tests, `StampToken` snapshot round-trip/signature-exclusion/backward-compat), `qr_scanner_screen_stamp_snapshot_test.dart` (6 tests, full directional-policy coverage via `QRScannerScreen.handleQRCodeForTesting`, Express Mode), `supplier_settings_profile_editors_test.dart` (6 tests, all three new editors + Stamps Required's now-general editability), `audit_trail_repository_test.dart` (7 tests), plus new migration-safety tests on both apps (customer v8→v9, supplier v5→v6). Full suite: shared 216/216, customer_app 137/137, supplier_app 97/100 - the 3 failures are in `supplier_redeem_card_test.dart` (Secure Mode redemption), confirmed **pre-existing and unrelated** to this work (reproduce identically against the unmodified baseline via `git stash`). `flutter analyze` clean across all three packages.
