...
- **Open specification gap, not yet resolved (flagged by the user 2026-08-23, discussion deliberately deferred):** a card that is *already complete* (fully stamped, awaiting redemption) when the business decreases `stampsRequired` is never adjusted, and - unlike an in-progress card - has no further stamp scan of its own to pick up a lower value on, since scanning a stamp only credits the *next* card once this one redeems. Worth also checking whether `RedemptionToken`/`RedemptionRequestToken` would need their own snapshot field for this to ever reach the completed-but-unredeemed card at all, since only `StampToken` currently carries one. Not a regression - simply never specified. Revisit `docs/project-management/Requirements/DISCUSSION_Business_Field_Editing.md` §4.1 when picked back up.
