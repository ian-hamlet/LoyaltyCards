...
### v2.2.4+40 - Build 40 (🟢 LIVE ON THE APP STORE; Android PAUSED pending Closed testing)
- **Date:** September 4, 2026 (Android merged to `develop` and real-device tested; release branch cut, iOS IPAs and macOS app built/verified, uploaded to TestFlight, and submitted for App Store review, all the same day); **September 6, 2026 - approved by Apple and released to the App Store** (both apps), TestFlight real-device testing completed with behavior confirmed consistent with prior versions; same day, Android Play Store submission work **paused** on discovering a mandatory Closed testing requirement (see below); `main` updated by merge the same day, by deliberate decision, ahead of Android clearing its own review.
- **Platform:** First release branch to cover iOS and Android together.
  - **iOS:** 🟢 **LIVE ON THE APP STORE** (both apps) - built via `build_both_apps_ios.sh`, verified (valid IPAs, correct 2.2.4/40 in `Info.plist`, correct bundle IDs), uploaded to TestFlight via Transporter, submitted for App Store review 2026-09-04, TestFlight-tested in parallel with the review (given how small the iOS-visible delta is this release - see Focus below), and **approved by Apple and released to the App Store 2026-09-06** - real-device testing found no regressions, behavior consistent with prior versions.
  - **Android:** 🟡 **PAUSED 2026-09-06.** Both apps live on Google Play **Internal testing** (not TestFlight - Play's equivalent, no review gate) - installed and functionally tested (Express + Secure Mode issue/stamp/redeem, biometric-gated Recovery Backup/Clone to Another Device) on two real Samsung Galaxy devices (A14/A12), the project's first real Android hardware. Play Store submission work was underway live (Customer app: Content Rating completed, Ads confirmed, Target audience/Data Safety mid-wizard) when a hard, previously-unknown requirement surfaced: personal developer accounts created after 13 November 2023 (this one, registered 2026-09-04) must run a **Closed testing** track - 12 testers opted in continuously for 14 days - before Production access is available at all; Internal testing does not count toward it. This corrects an earlier wrong claim in `ANDROID_APP_SUBMISSION_CHECKLIST.md` that Play review is "largely automated, often hours" - confirmed via Google's own Help Centre documentation that even the post-testing production-access review itself is "seven days or less, typically," not hours. Realistic floor from here: ~3 weeks once 12 testers are actually recruited and opted in. Paused pending a tester-recruitment plan - see `ANDROID_APP_SUBMISSION_CHECKLIST.md` and `ANDROID_PORT_PLAN.md` for full detail and exact resume state.
  - **macOS:** Supplier app release build verified (`flutter build macos --release` - universal x86_64+arm64 binary, correctly signed, correct version embedded). Not a store submission target.
- **Branch:** merged `feature/android-port` → `develop` (`767f641`, explicit merge commit - `develop` had not diverged at all, zero conflicts), release branch `releases/v2.2.4-build40` cut from `develop`. **Merged to `main` 2026-09-06** by deliberate decision - iOS is live and Android's remaining work is now a multi-week external gate (Closed testing), not something worth holding the whole release branch open for. `develop`, `releases/v2.2.4-build40`, and `main` are all kept on the same commit going forward.
- **Version:** 2.2.4+40
- **Status:** 🟢 iOS LIVE on the App Store (approved and released 2026-09-06). Android PAUSED - Internal testing real-device verified, Play Store submission on hold pending the Closed testing requirement above.
- **Focus:** The full Android port (native build/signing/release pipeline, real app icons/display names replacing Flutter's placeholders, Play Store metadata/graphics/submission checklist drafted) plus three real bugs found and fixed along the way: two Android-only (a biometric-auth crash needing `FlutterFragmentActivity`, an anti-fraud device-signal bug that hashed a non-per-device OS build tag instead of a real per-install identifier) and one affecting both platforms equally, just never visible on iOS screens before (a Secure Mode redemption screen showing two buttons that did the same thing - the one genuinely iOS-user-visible change this release). Also fixed, this session: a leftover Intel Homebrew `openssl` shadowing the native arm64 one, which was blocking every iOS/macOS build on this machine (`openssl x509 -subject` failing without Rosetta) - disabled the stale symlink rather than installing Rosetta as a workaround, and downloaded Xcode's missing iOS 26.5 device platform component. Full detail: `source/shared/lib/version.dart` Builds 37-40, `docs/project-management/ANDROID_PORT_PLAN.md`, `docs/deployment/APP_STORE_METADATA_PACKET_v2_2_4_40.md`.
- **Testing:** shared 216/216, customer_app 184/184 (+8 skipped), supplier_app 151/151 (+4 skipped). `flutter analyze` clean on both apps. Android: full Express/Secure Mode real-device pass on two Samsung Galaxy devices, 2026-09-04. iOS: TestFlight real-device testing completed 2026-09-06 - behavior confirmed consistent with prior versions, no regressions found, approved by Apple without review questions.
- **Defect found during this release's real-device testing (unrelated to Android/iOS itself):** TEST-023 - Clone to Another Device and Recovery Backup restore both silently reset a business's icon to the default Store icon (missing `logoIndex` field in `SupplierConfigBackup`, not a range bug - see `docs/project-management/DEFECT_TRACKER.md`). Deferred to after this release cycle, by user decision - does not block either store's release.
- **Next Steps:** Decide on a plan to recruit 12 genuine Closed-testing testers, then run the mandatory 14-day window before Android can even apply for production access - see `ANDROID_APP_SUBMISSION_CHECKLIST.md`'s "Closed Testing Requirement" section. The Customer app's App content declarations and Main store listing can continue in parallel (not blocked by the testing gate itself) - exact resume state is recorded in that same document. Repeat for the Supplier app once the Customer app is through (answers already drafted). Pick up TEST-023 once Android is unblocked.

### v2.2.1+36 - Build 36 (🟢 LIVE ON THE APP STORE)
- **Date:** August 27, 2026 (built, uploaded, TestFlight-tested, and submitted); August 29, 2026 (approved and released)
- **Platform:** Built and uploaded to App Store Connect via TestFlight, both apps. Extensive real-device TestFlight testing 2026-08-27: confirmed the print-preview hang on a real iPhone (100% reproducible on the +35 TestFlight build), confirmed the fix on a real iPhone and a real iPad Pro (M5), and confirmed via a controlled adjacent-version TestFlight update test (installed +35, updated in-place to +36) that the fix - not an update-delivery-pipeline fluke - resolves it. Metadata entered from `APP_STORE_METADATA_PACKET_v2_2_1_36.md`, both apps, submitted for App Store review 2026-08-27, and **approved by Apple and released to the App Store 2026-08-29** (both apps, no review questions raised).
- **Branch:** merged `fix/ios-print-sharepdf-hang` (branched from `feature/android-port`) → `develop` → `main` (fast-forward), release branch `releases/v2.2.1-build36` cut - all three branches on the same commit.
- **Version:** 2.2.1+36
- **Status:** 🟢 LIVE — available for download on the App Store. **Build-only bump** from v2.2.1+35 - the fix was originally intended as a version-only bump for on-device diagnosis, but the print-hang fix landed in the same commit once ready. Supersedes v2.2.0+32 as the current live version.
- **Focus:** Switches the Supplier app's four print call sites (`ConfigBackupService.printBackup`, `IssueCardBackupService.printIssueCard`, `SimpleTokenBackupService.printSimpleToken`, `AuditTrailPdfService.printAuditTrail`) from `Printing.layoutPdf` to `Printing.sharePdf`, avoiding the native `UIPrintInteractionController`/`CGPDFDocumentGetNumberOfPages` subsystem behind the CRASH-001 crash originally fixed in v2.0.2+21 - `layoutPdf` was found hanging 100% of the time on a real-device TestFlight build. Trade-off: the OS share sheet's built-in Print option has a simpler native print-options screen (no Scaling control) than the old panel. Also carries forward v2.2.1+33 (a real storage-error detection fix for backup/print paths, plus a large controller-extraction refactor), v2.2.1+34 (CocoaPods → Swift Package Manager migration, `app_settings`/`flutter_secure_storage` updates, ahead of Android work), and v2.2.1+35 (PDF text rendering fix for business names with accented Latin, Cyrillic, or Greek characters) - none of the three was submitted individually. Full detail: `source/shared/lib/version.dart` (Build 33-36 entries), `docs/technical/PACKAGE_UPDATE_MIGRATION_2026-08-26.md`, `docs/archive/project-management/CRASH-001-stamp-print-race-condition.md`.
- **Testing:** shared 216/216, customer_app 186/186, supplier_app 141/141. `flutter analyze` clean of errors across all three packages. Real-device TestFlight validation as described above (Platform, above).
- **Next Steps:** None - live on the App Store. Monitor App Store Connect for crash reports/reviews as usual.

### v2.2.1+33 - Build 33 (🟡 Superseded by v2.2.1+36, TestFlight-only, never submitted)
- **Date:** August 25, 2026
- **Platform:** Built and uploaded to TestFlight (not App Store review) on `feature/code-quality-refactor`/`develop`. Superseded before `main` was ever touched from this branch - `feature/android-port` (v2.2.1+34/+35) and `fix/ios-print-sharepdf-hang` (v2.2.1+36) picked up from `develop` afterward.
- **Branch:** `feature/code-quality-refactor` merged into `develop` (fast-forward); superseded by v2.2.1+36 above (merged to `main`).
- **Version:** 2.2.1+33
- **Status:** 🟡 Superseded by v2.2.1+36 above, never submitted for App Store review. **Patch version bump** (2.2.0 -> 2.2.1) on top of v2.2.0+32 - not build-only, since splitting `backup_storage_service.dart` surfaced a real bug present in the currently-live 2.2.0+32 print paths (missing disk-full detection - see Focus below); the controller-extraction refactor and docs consolidation are otherwise internal quality work, and the marketing-file sync isn't app code at all. v2.2.1+34 (SPM/package migration) and v2.2.1+35 (PDF font fix) followed on `feature/android-port` before v2.2.1+36 consolidated everything for submission - see that entry above.
