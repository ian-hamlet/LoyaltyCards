...
**Target Release:** 🟢 v2.2.4+40 built, uploaded to TestFlight, TestFlight-tested, submitted for App Store review 2026-09-04, and **approved by Apple and released to the App Store 2026-09-06** (both apps, no review questions raised). Real-device TestFlight testing found behavior consistent with prior versions, no regressions. Release branch `releases/v2.2.4-build40` cut from `develop` (also carries the Android port, still pending Play Store submission - not merged to `main` yet, held until both platforms clear review). Supersedes v2.2.1+36 as the current live version. First iOS build since v2.2.1+36 - v2.2.2+37/v2.2.3+38/v2.2.4+39 were Android-only fixes, never built for iOS at all.  
**Last Updated:** September 6, 2026

**Status note:** 🟢 **v2.0.2+21 was live on the App Store** (shipped 2026-08-10 — see prior status below for that history), now superseded. 🟢 **v2.0.3+23 was submitted 2026-08-15, approved and released 2026-08-16** (both apps) - the Sharing feature and both bug fixes are confirmed working. Metadata from `APP_STORE_METADATA_PACKET_v2_0_3_23.md` entered into ASC, build 23 selected on both apps, Release was set to **Manual** on both (the two apps review at different speeds, so release was held until both were approved). **⚠️ This live build contains TEST-016** (businesses with 3 or 4 required stamps can't issue a valid card) - see `docs/project-management/DEFECT_TRACKER.md`. 🟡 **v2.1.0+26 fixed TEST-016 plus TEST-017 through TEST-020 and was built and uploaded to TestFlight**, but not submitted for App Store review before TEST-021 was found on that same TestFlight build - see `docs/project-management/DEFECT_TRACKER.md` TEST-021. 🟢 **v2.1.0+27 added TEST-021 and was built and uploaded to TestFlight** the night of 2026-08-16/17. 🟢 **v2.1.1+28 added DECISION-017 and was also built and uploaded to TestFlight**, real-device verified end-to-end - but that same testing round found TEST-022 (see below). 🟢 **v2.1.1+29 added the TEST-022 fix** (plus DECISION-019's dedup/dead-code cleanup) - build-only bump, built and delivered to TestFlight 2026-08-18, real-device validated, metadata confirmed in ASC, submitted for App Store review 2026-08-18 (both apps), and **approved by Apple and released to the App Store 2026-08-19** (both apps) - superseded below. 🔵 **v2.2.0+30 adds an editable Express Mode scan cooldown** (Supplier app Settings) - minor version bump, a genuine capability increase rather than a bug fix. Folded into v2.2.0+32 below, never built or uploaded on its own. 🟢 **v2.2.0+32 consolidates v2.2.0+30 and v2.2.0+31** (neither ever built/uploaded) plus a macOS desktop port and real-device-found fixes - submitted for App Store review 2026-08-23, and **approved by Apple and released to the App Store 2026-08-24** (both apps) - superseded by v2.2.1+36 below. 🟡 **v2.2.1+33 through +35 were TestFlight-only validation rounds** on `feature/android-port` (package/toolchain updates ahead of Android work, a font-rendering fix, and a print-hang investigation) - never submitted for App Store review. **v2.2.1+36 fixes the print hang** (switched from `Printing.layoutPdf` to `Printing.sharePdf`, avoiding the native subsystem behind CRASH-001) - built, uploaded to TestFlight, TestFlight-tested, merged `fix/ios-print-sharepdf-hang` → `develop` → `main`, release branch `releases/v2.2.1-build36` cut, **submitted for App Store review 2026-08-27**, and **approved by Apple and released to the App Store 2026-08-29** (both apps) - superseded below. 🟢 **v2.2.2+37 through v2.2.4+39 were Android-only fixes** (biometric-auth crash, an anti-fraud device-signal bug, a duplicate redemption button) built and version-bumped as part of the Android port (`feature/android-port`) but **never built for iOS at all** - see `docs/project-management/ANDROID_PORT_PLAN.md`. **v2.2.4+40 carries the duplicate-button fix forward to iOS** (the only one of the three with any iOS code path) - built, uploaded to TestFlight, TestFlight-tested in parallel with the review given how small the iOS delta is, release branch `releases/v2.2.4-build40` cut from `develop`, **submitted for App Store review 2026-09-04**, and **approved by Apple and released to the App Store 2026-09-06** (both apps, no review questions raised) - **the current live version.**

**Version history since v1.0.3+11** (the version this checklist was previously verified against): v1.6.0+16/+17 added app-wide biometric lock to the supplier app and required device auth before committing a business restore/clone, merged into `develop` via `feature/uireview`; v2.0.0+18 was a **major version bump** for a breaking QR token format change (new signed fields added during a security review mean pre-review printed QR codes fail signature verification against the new signed data — acceptable since the app has never had real-world users yet); v2.0.0+19 fixed a critical redemption-inflation gap and a repeat-customer lockout bug, renamed "Simple Mode" to "Express Mode" throughout all user-facing copy, and added the App Store metadata/public-site work described below — **submitted 2026-07-28, rejected for CRASH-001**; v2.0.1+20 fixed CRASH-001 (re-entrancy guard + PDF-bytes validation) and a dark-mode contrast bug (UI-001), but was never uploaded; v2.0.2+21 carried the same fixes plus the `IPHONEOS_DEPLOYMENT_TARGET` bump to 15.0 that Transporter required — **passed review and shipped 2026-08-10**; v2.0.3+22 added the App Store Category/Subtitle corrections found post-launch but never produced an uploaded build; v2.0.3+23 added the Sharing feature (Tell a Business / Tell a Friend, both apps) plus two bug fixes found during TestFlight-prep testing (Express Mode stamp routing, a false error on opening Clone/Recovery Backup screens) — **built, uploaded, TestFlight-tested, submitted for App Store review 2026-08-15, and approved and released 2026-08-16**, but contains TEST-016 (see below); v2.0.4+24 fixed TEST-016 but never shipped - folded into v2.1.0+26; v2.1.0+26 carried TEST-016 forward and added the real fix for a QR-capacity failure found while testing it (TEST-017: redemption QR could silently fail to render at high stamp counts; TEST-018: a related stamp-provenance bug found along the way; TEST-019: a clearer error message for an affected business; TEST-020: the actual fix - compact gzip+Base45+alphanumeric-mode QR encoding, raising the safe stamps-required ceiling from 10 to 12) - **minor version bump (2.0.4 -> 2.1.0), deliberate given the real capability increase** - **built and uploaded to TestFlight**, where TEST-021 was then found (the same QR-capacity failure, never fixed on the issue-card side); v2.1.0+27 added that fix and was itself built and uploaded to TestFlight; v2.1.1+28 added DECISION-017 (self-service recovery for an out-of-range business, closing the gap TEST-019 left open) - **patch version bump** - built and uploaded to TestFlight, real-device verified end-to-end, where that same testing surfaced TEST-022 (a cross-version compatibility regression from TEST-021); v2.1.1+29 added that fix plus DECISION-019 (dedup/dead-code cleanup) - **build-only bump**, built and delivered to TestFlight 2026-08-18, real-device validated, submitted for App Store review 2026-08-18, and **approved and released to the App Store 2026-08-19**; v2.2.0+30 added an editable Express Mode scan cooldown plus a planned positioning/metadata update - **minor version bump**, never built or uploaded on its own, folded forward; v2.2.0+31 added supplier-editable business profile fields, directional `stampsRequired` policy, and a local audit trail, also never built or uploaded on its own, folded forward; v2.2.0+32 consolidated both plus a macOS desktop port and a round of real-device-found fixes - **build-only bump**, built and delivered to TestFlight 2026-08-23, real-device validated, submitted for App Store review 2026-08-23, and **approved and released to the App Store 2026-08-24**, later superseded by v2.2.1+36 (see below); v2.2.1+33 was a **patch version bump** for a real bug fix (backup/print paths now correctly detect "not enough storage space" instead of a raw error) plus a large internal controller-extraction refactor (no behavior change) on `feature/code-quality-refactor`/`develop`, built and uploaded to TestFlight but never submitted; v2.2.1+34 was a **build-only bump** migrating both apps from CocoaPods to Swift Package Manager and updating `app_settings`/`flutter_secure_storage`, ahead of Android work on `feature/android-port` - no app-visible change, built and uploaded to TestFlight but never submitted; v2.2.1+35 was a **build-only bump** fixing PDF text rendering for business names with accented Latin, Cyrillic, or Greek characters (the default font was WinAnsi-only), built and uploaded to TestFlight but never submitted - real-device TestFlight testing of this build is what surfaced the print-hang investigated next; v2.2.1+36 is a **build-only bump** that switches the Supplier app's four print flows from `Printing.layoutPdf` to `Printing.sharePdf`, fixing a print-preview hang confirmed (via a controlled adjacent-version TestFlight update test on a real iPhone) to be caused by the same native subsystem behind the CRASH-001 crash fixed in v2.0.2+21 - **built, uploaded to TestFlight, TestFlight-tested, merged to `develop` and `main`, release branch `releases/v2.2.1-build36` cut, submitted for App Store review 2026-08-27, and approved by Apple and released to the App Store 2026-08-29**, later superseded by v2.2.4+40 (see below); v2.2.2+37, v2.2.3+38, and v2.2.4+39 were each real bug fixes found during Android-port work (biometric-auth crash, an anti-fraud device-signal bug, a duplicate redemption button) but **none was ever built for iOS** - version-bumped in source only, built and tested exclusively via Play Console/Android hardware; v2.2.4+40 carries v39's duplicate-button fix (the only one of the three touching any iOS code path - the other two are Android-only files) forward into an actual iOS build - **build-only from an iOS-code perspective relative to v2.2.1+36, though a real patch-version fix on Android** - built via `build_both_apps_ios.sh`, uploaded to TestFlight, TestFlight-tested in parallel with the review (justified given how small the iOS delta is), release branch `releases/v2.2.4-build40` cut from `develop` (also carrying the full Android port, not yet merged to `main`), **submitted for App Store review 2026-09-04, and approved by Apple and released to the App Store 2026-09-06** (both apps, no review questions raised) - **the current live version.**
...
**v2.2.4+40 — LIVE ON THE APP STORE, approved and released 2026-09-06:**

- [x] **Final build number incremented** across all three `pubspec.yaml` files + `source/shared/lib/version.dart` — `2.2.4+40`, confirmed in sync
- [x] **Version number confirmed** — v2.2.4+40 (patch bump from v2.2.1+36 on iOS - carries v2.2.4+39's duplicate-button fix, the only one of the three Android-cycle fixes touching any iOS code path; v2.2.2+37/v2.2.3+38/v2.2.4+39 themselves were never built for iOS at all)
- [x] **All code merged** — `feature/android-port` → `develop` (`767f641`, explicit merge commit, zero conflicts), release branch `releases/v2.2.4-build40` cut from `develop`. Not yet merged to `main` - held until Android also clears Play Store review
- [x] **Archive builds completed** for v2.2.4+40 — built via `build_both_apps_ios.sh`, both apps
- [x] **IPA files verified** — valid zip, correct 2.2.4/40 in `Info.plist`, correct bundle IDs, both apps
- [x] **IPA files uploaded to App Store Connect** via Transporter — both apps, 2026-09-04
- [x] **TestFlight testing completed** — real-device testing found behavior consistent with prior versions, no regressions, confirmed the Customer app's Secure Mode redemption screen now shows only the single "Scan Redemption" button
...
- [x] **Critical bugs resolved** — no CRITICAL/HIGH defects open against this build. TEST-023 (business icon lost on Clone/Recovery restore, found during Android real-device testing) logged as MEDIUM priority and deliberately deferred to after this release cycle - see `docs/project-management/DEFECT_TRACKER.md`, not a regression from any prior iOS version
- [x] **Metadata entered into App Store Connect** — What's New and App Review Notes from `APP_STORE_METADATA_PACKET_v2_2_4_40.md`, both apps; Subtitle/Promotional Text/Keywords/Description unchanged from v2.2.1+36 (Promotional Text re-entered anyway - confirmed expected ASC behavior that it clears on every release, not a bug)
- [x] **Submitted for App Store review** — 2026-09-04, both apps
- [x] **Approved and released** — approved by Apple and released 2026-09-06 (both apps, no review questions raised), now **LIVE on the App Store**
...
**v2.2.0+32 — LIVE ON THE APP STORE, approved and released 2026-08-24:**
...
- [x] **Archive builds completed** for v2.2.0+32 — both apps
- [x] **IPA files uploaded to App Store Connect** via Transporter — both apps
- [x] **Build processing complete** in App Store Connect — confirmed, build 32 available on TestFlight
...
- [x] **Approved and released** — approved by Apple and released 2026-08-24 (both apps, via App Store Connect), now **LIVE on the App Store**
- [x] **v2.1.1+29 status** — already approved and released 2026-08-19; no defect to supersede this time, this is a feature release; v2.1.1+29 is now superseded by v2.2.0+32 as the current live version

---

**v2.2.1+33 / +34 / +35 (shipped to TestFlight only, never submitted for App Store review, superseded by v2.2.1+36):**

- v2.2.1+33: patch version bump (real bug fix, not build-only) - backup/print paths now correctly detect "not enough storage space" instead of a raw error message, plus a large internal controller-extraction refactor (no behavior change). Built on `feature/code-quality-refactor`/`develop`.
- v2.2.1+34: build-only bump - migrated both apps from CocoaPods to Swift Package Manager, updated `app_settings` and `flutter_secure_storage`, ahead of Android work. No app-visible change. Built on `feature/android-port`. See `docs/technical/PACKAGE_UPDATE_MIGRATION_2026-08-26.md`.
- v2.2.1+35: build-only bump - fixed PDF text rendering for business names with accented Latin, Cyrillic, or Greek characters. Built on `feature/android-port`. Real-device TestFlight testing of this specific build is what surfaced the print-preview hang investigated and fixed in v2.2.1+36.
- None of the three was submitted for App Store review - each was superseded by the next before reaching that stage. **Should not be submitted for App Store review as-is** (all three still use the print-hang-prone `Printing.layoutPdf` path fixed in +36).

**v2.2.1+36 (approved by Apple and released to the App Store 2026-08-29 — now LIVE on the App Store):**

- [x] **Final build number incremented** in pubspec.yaml (both apps + shared) — `2.2.1+36`, confirmed in `source/{customer_app,supplier_app,shared}/pubspec.yaml` and `source/shared/lib/version.dart`
- [x] **Version number confirmed** — v2.2.1+36 (build-only bump from 2.2.1+35 - the fix below was originally intended as a version-only bump for on-device diagnosis, but the print-hang fix landed in the same commit once ready; version.dart's Build 36 entry corrected 2026-08-27 to reflect this)
- [x] **Feature complete for this build's purpose** — Supplier app's four print call sites (`ConfigBackupService.printBackup`, `IssueCardBackupService.printIssueCard`, `SimpleTokenBackupService.printSimpleToken`, `AuditTrailPdfService.printAuditTrail`) switched from `Printing.layoutPdf` to `Printing.sharePdf`, avoiding the native `UIPrintInteractionController`/`CGPDFDocumentGetNumberOfPages` subsystem behind CRASH-001. Also carries the +33 storage-error fix, +34 package/SPM migration, and +35 font fix forward (none resubmitted individually - see the +33/+34/+35 entry above). Full detail: `source/shared/lib/version.dart` Build 33-36 entries, `docs/archive/project-management/CRASH-001-stamp-print-race-condition.md`.
- [x] **All code merged to `develop`** — 2026-08-27, fast-forward from `fix/ios-print-sharepdf-hang`
- [x] **`develop` merged to `main` branch** — 2026-08-27, fast-forward
- [x] **Release branch created** `releases/v2.2.1-build36` — 2026-08-27, all three branches (`main`/`develop`/`releases/v2.2.1-build36`) on the same commit
- [x] **Archive builds completed** for v2.2.1+36 — both apps
- [x] **IPA files uploaded to App Store Connect** — both apps, confirmed on TestFlight
- [x] **Build processing complete** in App Store Connect — confirmed, build 36 available on TestFlight
- [x] **Physical-device verification completed** — extensive real-device TestFlight testing 2026-08-27: confirmed the print hang on a real iPhone (100% reproducible on the +35 TestFlight build), confirmed the fix on a real iPhone and a real iPad Pro (M5), and confirmed via a controlled adjacent-version TestFlight update test (installed +35, updated in-place to +36) that the fix - not an update-delivery-pipeline fluke - resolves it
- [x] **All automated tests passing** (shared 216, customer_app 186, supplier_app 141)
- [x] **`flutter analyze` clean** of errors on all three packages (pre-existing lint-level info/warnings remain, unchanged by this work)
- [x] **Metadata entered into App Store Connect** — from `APP_STORE_METADATA_PACKET_v2_2_1_36.md`, both apps, 2026-08-27
- [x] **Submitted for App Store review** — ✅ 2026-08-27, both apps
- [x] **Approved and released** — approved by Apple and released 2026-08-29 (both apps, via App Store Connect), now **LIVE on the App Store**
- [x] **v2.2.0+32 status** — approved and released 2026-08-24; superseded by v2.2.1+36 above, approved and released 2026-08-29, as a reliability/bug-fix release (no new App-Store-facing capability)

---

**v2.1.1+29 — LIVE ON THE APP STORE, approved and released 2026-08-19:**
...
- [x] **All fixes complete** — TEST-016 through TEST-022 plus DECISION-017, on top of `fix/TEST-017-redemption-qr-overflow`'s prior work; DECISION-019 (N-008/N-009 dedup, dead-code removal) also shipped in this build
- [x] **All code merged to `develop`** — TEST-016 through DECISION-017 as above; TEST-022 fix and DECISION-019 committed and pushed 2026-08-17 (`68fd740`)
- [x] **`develop` merged to `main` branch** — done 2026-08-18, fast-forward merge, all three branches at the same commit
- [x] **Release branch created** `releases/v2.1.1-build29` — done 2026-08-18, branched from the same commit as `develop`/`main`
...
- [x] **Metadata entered into App Store Connect** — What's New, both apps' Promotional Text, and Manual release confirmed - see `APP_STORE_METADATA_PACKET_v2_1_1_29.md`; all other fields unchanged from v2.0.3+23 and already live
- [x] **Submitted for App Store review** — ✅ 2026-08-18, both apps
- [x] **Approved and released** — ✅ approved by Apple and released 2026-08-19 (both apps, via App Store Connect), now **LIVE on the App Store**
- [x] **v2.0.3+23 status** — already approved and released 2026-08-16; v2.1.1+29 now live to supersede it and close out TEST-016 (live in v23)
...
- [x] **v2.2.1+36 What's New, App Review Notes** — entered into ASC 2026-08-27 from [`APP_STORE_METADATA_PACKET_v2_2_1_36.md`](APP_STORE_METADATA_PACKET_v2_2_1_36.md), both apps. Subtitle/Keywords/Description unchanged from v2.2.0+32.
...
9. [x] **Release approved apps** — both apps manually released 2026-08-16, live on the App Store. ⚠️ Contains TEST-016 - v2.1.1+29 (the fix) submitted for App Store review 2026-08-18, see below.
...
5. [x] **Screenshots** — unchanged, no screens affected (the editable-cooldown and business-profile-editor UI are Settings-screen additions, not part of the staged screenshot set)
6. [x] **Submit for review** — both apps, confirmed 2026-08-23
7. [x] **Monitor review status** — ✅ Apple approved both apps 2026-08-24
8. [x] **Respond to App Review** if questions arise — not needed, no questions raised
9. [x] **Release approved apps** — ✅ Manual release, both apps released together 2026-08-24, same reasoning as prior versions

---

**v2.2.1+33 / +34 / +35 (shipped to TestFlight only, never submitted for App Store review, superseded by v2.2.1+36):**

1. [x] **Upload build to App Store Connect** (via TestFlight) — builds 33, 34, 35, both apps
2. [ ] **Select build** for Customer app submission — not submitted; each superseded before this step
3. [ ] **Select build** for Supplier app submission — same
4. [ ] **Complete all required fields** in App Store Connect
5. [ ] **Screenshots**
6. [ ] **Submit for review** — deliberately not done; each still carried the print-hang-prone `Printing.layoutPdf` path fixed in +36
7. [ ] **Monitor review status**
8. [ ] **Respond to App Review** if questions arise
9. [ ] **Release approved apps**
...
1. [x] **Upload build to App Store Connect** (via TestFlight) — build 36, both apps — confirmed
2. [x] **Select build** for Customer app submission — build 36
3. [x] **Select build** for Supplier app submission — build 36
4. [x] **Complete all required fields** in App Store Connect — metadata entered from `APP_STORE_METADATA_PACKET_v2_2_1_36.md`, both apps, 2026-08-27
5. [x] **Screenshots** — unchanged, no screens affected (the print flow's UI change - share sheet instead of a direct print-preview panel - isn't part of the staged screenshot set, which doesn't show the print dialog itself)
6. [x] **Submit for review** — ✅ both apps, 2026-08-27, after merging `fix/ios-print-sharepdf-hang` → `develop` → `main` and cutting release branch `releases/v2.2.1-build36`
7. [x] **Monitor review status** — approved 2026-08-29
8. [x] **Respond to App Review** if questions arise — n/a, approved without questions
9. [x] **Release approved apps** — both apps released 2026-08-29, live on the App Store, superseding v2.2.0+32
...
2. [x] **Select build** for Customer app submission — build 29
3. [x] **Select build** for Supplier app submission — build 29
...
5. [x] **Screenshots** — unchanged, no screens affected by these fixes (QR appearance is visually similar regardless of underlying encoding; DECISION-017's banner/dialog are new UI, but not part of the staged screenshot set)
6. [x] **Submit for review** — ✅ submitted 2026-08-18, both apps
7. [x] **Monitor review status** — ✅ Apple approved both apps 2026-08-19
8. [x] **Respond to App Review** if questions arise — not needed, no questions raised
9. [x] **Release approved apps** — ✅ Manual release, both apps released together 2026-08-19, same reasoning as v2.0.3+23
...
**Document Status:** 🟢 **v2.1.1+29 is LIVE ON THE APP STORE** (both apps), submitted for App Store review 2026-08-18, **approved and released 2026-08-19**, superseding v2.0.3+23 (which was live but contained TEST-016). **v2.0.3+23** was submitted 2026-08-15, approved and released 2026-08-16, superseding v2.0.2+21 (shipped 2026-08-10 — the project's first public release). ⚠️ **v2.0.3+23 contained TEST-016** (businesses with 3 or 4 required stamps can't issue a valid card) - fix shipped to **TestFlight as v2.1.0+26** (minor version bump, also fixes TEST-017 through TEST-020, a redemption QR-capacity failure found while testing TEST-016 and everything it surfaced along the way), but not submitted for App Store review since TEST-021 (the issue-card counterpart to TEST-017/020's fix) was found on that same TestFlight build. **v2.1.0+27** added TEST-021 and was built and uploaded to TestFlight. **v2.1.1+28** (patch version bump - DECISION-017 is a genuine UX improvement, not build-only) added a self-service fix for a business outside the supported stamps-required range, also built and uploaded to TestFlight, real-device verified end-to-end - see `docs/testing/DECISION-017_LEGACY_BUSINESS_TEST_PLAN.md`. That same testing found **TEST-022**, a cross-version compatibility regression from TEST-021. **v2.1.1+29** (build-only bump) adds that fix plus DECISION-019 (dedup/dead-code cleanup) - built and delivered to TestFlight 2026-08-18, real-device validation and ASC metadata (Promotional Text both apps, Manual release both apps) confirmed 2026-08-18, submitted for App Store review 2026-08-18 (both apps), and **approved by Apple and released to the App Store 2026-08-19** (both apps) - superseded below. 🟢 **v2.2.0+32 (LIVE ON THE APP STORE)** consolidates v2.2.0+30 (editable Express Mode scan cooldown) and v2.2.0+31 (editable business profile fields, directional `stampsRequired` policy, local audit trail) - neither ever built or uploaded - plus a macOS desktop build for the Supplier app, a real-device-found `stampsRequired` fix, and test-suite quality fixes. **Build-only bump** relative to +31. Merged `feature/businessedit` → `develop` → `main` 2026-08-23, release branch `releases/v2.2.0-build32` cut, built and uploaded to TestFlight the same day, basic TestFlight testing completed the same day, metadata entered and both apps submitted for App Store review the same day, and **approved by Apple and released to the App Store 2026-08-24** (both apps) - superseded by v2.2.1+36 below. See `RELEASES.md` for the release-branch record, `docs/project-management/DEFECT_TRACKER.md` DECISION-021/DECISION-022 for the defects, and `CHANGELOG.md` [2.2.0+32] for the full change list. 🟡 **v2.2.1+33 through +35** were TestFlight-only validation rounds on `feature/android-port` (a real storage-error bug fix plus a large refactor in +33, a package/SPM migration in +34, a PDF-font/Unicode fix in +35) - none submitted for App Store review. **v2.2.1+36** switches the Supplier app's print flows from `Printing.layoutPdf` to `Printing.sharePdf`, fixing a print-preview hang confirmed via real-device TestFlight testing (including a controlled adjacent-version update test isolating the fix from update-pipeline noise) to be caused by the same native subsystem behind the CRASH-001 crash originally fixed in v2.0.2+21. Built, uploaded to TestFlight, and TestFlight-tested on `fix/ios-print-sharepdf-hang` (branched from `feature/android-port`), then merged → `develop` → `main`, release branch `releases/v2.2.1-build36` cut (all three branches on the same commit), metadata entered into ASC from `APP_STORE_METADATA_PACKET_v2_2_1_36.md`, submitted for App Store review 2026-08-27, and **approved by Apple and released to the App Store 2026-08-29** (both apps) - superseded below. See `source/shared/lib/version.dart` (Build 33-36 entries) and `docs/archive/project-management/CRASH-001-stamp-print-race-condition.md` for full technical detail. 🟢 **v2.2.2+37 through v2.2.4+39** were real bug fixes found during Android-port work (biometric-auth crash, an anti-fraud device-signal bug, a duplicate redemption button) but were **never built for iOS** - version-bumped in source only, tested exclusively via Play Console/Android hardware, see `docs/project-management/ANDROID_PORT_PLAN.md`. **v2.2.4+40** carries v39's duplicate-button fix (the only one of the three touching any iOS code) into an actual iOS build - built via `build_both_apps_ios.sh`, uploaded to TestFlight, TestFlight-tested in parallel with the review, release branch `releases/v2.2.4-build40` cut from `develop` (also carrying the full Android port - not yet merged to `main`, held until Android clears Play Store review too), metadata entered from `APP_STORE_METADATA_PACKET_v2_2_4_40.md`, submitted for App Store review 2026-09-04, and **approved by Apple and released to the App Store 2026-09-06** (both apps, no review questions raised) - **🟢 LIVE ON THE APP STORE, the current live version.** One defect (TEST-023 - a business icon lost on Clone/Recovery restore, unrelated to iOS or this release's own changes) was found during this cycle's Android real-device testing and deliberately deferred - see `docs/project-management/DEFECT_TRACKER.md`.  
