...
- **Version:** 2.2.1+36
- **Status:** 🔵 SUBMITTED - awaiting Apple's review decision. **Build-only bump** from v2.2.1+35 - the fix was originally intended as a version-only bump for on-device diagnosis, but the print-hang fix landed in the same commit once ready. Supersedes v2.2.0+32 (the current live version) once approved and released.
...
- **Next Steps:** Monitor App Store Connect for Apple's review decision.
