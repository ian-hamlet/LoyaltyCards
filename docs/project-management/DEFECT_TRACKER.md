...
- **Notes:** Merged `feature/businessedit` → `develop` → `main` 2026-08-23 (fast-forward), release branch `releases/v2.2.0-build32` cut, built and uploaded to TestFlight (both apps) 2026-08-23, basic TestFlight testing completed the same day, metadata entered and both apps submitted for App Store review the same day - awaiting Apple's decision. Full design discussion, including the rejected alternatives for the `stampsRequired` propagation question, lives in `docs/project-management/Requirements/DISCUSSION_Business_Field_Editing.md`.
