...
### v2.2.4+40 - Build 40 (🟢 Submitted for App Store review; live on Google Play Internal testing)
- **Date:** September 4, 2026 (Android merged to `develop` and real-device tested; release branch cut, iOS IPAs and macOS app built/verified, uploaded to TestFlight, and submitted for App Store review, all the same day)
- **Platform:** First release branch to cover iOS and Android together.
  - **Android:** both apps live on Google Play **Internal testing** (not TestFlight - Play's equivalent, no review gate) - installed and functionally tested (Express + Secure Mode issue/stamp/redeem, biometric-gated Recovery Backup/Clone to Another Device) on two real Samsung Galaxy devices (A14/A12), the project's first real Android hardware. This is `feature/android-port`'s first release since merging to `develop`.
  - **iOS:** both apps built via `build_both_apps_ios.sh`, verified (valid IPAs, correct 2.2.4/40 in `Info.plist`, correct bundle IDs), uploaded to TestFlight via Transporter, and **submitted for App Store review 2026-09-04** (both apps) - TestFlight-tested in parallel with the review, given how small the iOS-visible delta is this release (see Focus below).
...
- **Branch:** merged `feature/android-port` → `develop` (`767f641`, explicit merge commit - `develop` had not diverged at all, zero conflicts), release branch `releases/v2.2.4-build40` cut from `develop`. Not yet merged to `main` - held until both apps clear App Store review, per this project's standard convention.
- **Version:** 2.2.4+40
- **Status:** 🟢 Android real-device tested via Play Console Internal testing; iOS submitted for App Store review, TestFlight testing in progress in parallel. Metadata entered from `APP_STORE_METADATA_PACKET_v2_2_4_40.md` (both apps).
...
- **Testing:** shared 216/216, customer_app 184/184 (+8 skipped), supplier_app 151/151 (+4 skipped). `flutter analyze` clean on both apps. Android: full Express/Secure Mode real-device pass on two Samsung Galaxy devices, 2026-09-04. iOS: build/signing verified; TestFlight real-device testing in progress alongside the App Store review (Apple's review has no code-freeze requirement during review, and the iOS delta this release is small enough to test in parallel rather than block submission on it).
- **Next Steps:** Monitor App Store review for both apps. Complete the Play Store listing in Play Console (content rating questionnaire, Data Safety form - both already decided, see `PLAY_STORE_METADATA_PACKET_v2_2_2_37.md` and `ANDROID_APP_SUBMISSION_CHECKLIST.md`), then submit Android for Play review. Merge `releases/v2.2.4-build40` → `main` once both platforms clear review.
