...
- **Notes:** Per explicit discussion, 12 was chosen deliberately over pursuing full bulletproofing at the original 20-stamp ceiling - the cost of closing the gap from "very safe" to "no known failure mode" turned out to be one well-specified codec and a QR-construction call, not a redesign, so it was worth doing for the 12-stamp target. A manual/human-verified redemption override remains a separate, still-open discussion for the cases this can't reach (an already-issued pre-TEST-020 card, or a business ever wanting to reconfigure beyond 12) - not a substitute for this fix, but not eliminated by it either.
