...
- ~~**New test coverage for**: ...~~ **DONE (2026-07-25)** — see `docs/quality/VULNERABILITIES.md`'s "Test Coverage Added" section for the full list of new test files. Closed on `feature/SecurityReview` alongside the vulnerability fixes: `database_helper.dart`, `stamp_repository.dart`, `transaction_repository.dart`, `qr_token_generator.dart` (both apps), `supplier_database_helper.dart`, and the `supplier_onboarding.dart` mode-selection UI (widget tests). Migration/backup/restore private methods remain untested (private, only reachable via a real version-bump scenario — noted as a deliberate gap, not an oversight).
