...
- **Testing Verified:** New `card_issue_qr_codec_test.dart` (10 tests - round-trip fidelity, Base45/JSON-incompatibility checks, version marker, and worst-case size verification up to 20 initial stamps fitting and 35 not fitting, against the real `qr` package). New widget test in `supplier_issue_card_test.dart` - a legacy 20-stamp business issuing a card with 17 and then 20 pre-applied stamps renders the QR successfully with no fallback panel. Full suite green: shared 211, customer_app 131, supplier_app 82. `flutter analyze` clean on every new/changed file.
- **Fix Branch:** `fix/TEST-017-redemption-qr-overflow` (off `develop`) - same branch as TEST-017 through TEST-020
- **Target Build:** v2.1.0+27 (build-only bump - v2.1.0+26 was already built and uploaded to TestFlight without this fix before it was found, and Apple doesn't allow re-uploading a build number with different content)
- **Notes:** Found by the user during real-device testing of the v2.1.0+26 TestFlight build's TEST-020 fix, using the same legacy 20-stamp business from earlier in this defect chain - a good example of why that business was worth keeping around for testing rather than resetting. Doesn't affect any business created under the current 3-12 range; only a legacy business with a stored `stampsRequired` above the current range can reach a high enough initial-stamp count to hit this.
