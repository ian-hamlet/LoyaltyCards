#!/usr/bin/env bash
#
# Runs the Customer app (LoyaltyCards) as a native macOS desktop app.
#
# Usage: from anywhere, run:
#   ./run_customer_app_macos.sh
# or:
#   bash source/run_customer_app_macos.sh
#
# Once running: r = hot reload, R = hot restart, q = quit, h = list all commands.
#
# Tip: run this alongside run_supplier_app_macos.sh to test the full P2P
