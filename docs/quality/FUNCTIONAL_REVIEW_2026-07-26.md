...
**Status:** 📋 DEFERRED — separate check needed, not part of this fix pass
...
**Deliberately excluded from this fix pass** — the manual rotation-offset buttons and saved preference exist because `mobile_scanner` previously couldn't reliably detect device orientation on its own. The package-update branch (`feature/packageUpdate`) already bumped `mobile_scanner` 7.2.0 → 7.4.0; whether that release fixed the underlying detection issue needs to be checked and tested independently (on a real device, likely the iPad specifically, since that's where the upside-down gap is reachable) before touching this code - not bundled into this fix pass. If the newer version does handle orientation correctly, the manual workaround and the dead `DeviceOrientationService` can likely be removed together in a follow-up.
