...
- **Printing (audit trail, backups, stamp/issue-card QR PDFs) failed with a generic "printing not available" error - fixed.** `supplier_app`'s macOS entitlements never had `com.apple.security.print`, so the App Sandbox blocked all access to the system print spooler regardless of what the Dart code did. Added to both `DebugProfile.entitlements` and `Release.entitlements`; verified present in a clean build via `codesign -d --entitlements - path/to/the.app`. Affected every print/print-to-PDF feature in the supplier app, not just the one the bug was first noticed on.
