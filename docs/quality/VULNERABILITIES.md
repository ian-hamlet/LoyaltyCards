...
- **Device clock manipulation (re-confirmed 2026-07-27):** `canReceiveStamp`'s elapsed-time check is computed from `DateTime.now()` (`rate_limiter.dart`) - pure OS wall-clock, user-adjustable via the device's own Settings app, no jailbreak or tooling required. Advancing the clock forward past the configured cooldown and scanning again resets it immediately; this is the same root cause as the two exploits above (nothing about the cooldown is cryptographically anchored), just a third concrete mechanism to reach it, surfaced by a fraud-focused re-review after the v2.0.0 security/UI pass.

**Decision (2026-07-25, reconfirmed 2026-07-27):** Resolved as **by design**, the same way as V-001. `scanInterval` is never cryptographically enforced in *any* mode - it's a client-side friction/UX signal, not a security boundary, even in Secure Mode. Signing it (as V-010 now does defensively for the fields that matter) wouldn't close the two-device bypass anyway, since Express Mode performs no signature verification of any kind - that's the whole point of Express Mode's honor-based design. The two-device bypass specifically is an inherent limitation of a P2P architecture with no server to check across devices, not a bug to fix - same category as V-009's card-revocation limitation. A real fix would mean adding server-side or cross-device state to Express Mode, contradicting its purpose (fast, no-equipment, trust-based checkout).

On re-review (2026-07-27), the device-clock exploit specifically was assessed against Express Mode's actual product framing - a digital stand-in for a physical paper stamp card, not a fraud-hardened payment instrument - and confirmed as an accepted, non-significant risk in that context: a paper card offers no better resistance to a dishonest customer, and the business decision to offer Express Mode already accepts that trade-off in exchange for zero-friction checkout. No further mitigation planned. Flagging this explicitly so a future review doesn't need to re-derive the reasoning: **the clock-manipulation angle was considered and deliberately not treated as a priority fix.**
